...
(function () {
  ...
  const CONFIG = {
    ...
    CSS_CLASSES: {
      ...
      TOAST_SHOW: "show",
      LIKE_BTN: "like-btn",
      LIKE_BTN_ACTIVE: "like-btn-active",
    },
    API_ENDPOINTS: {
      TOPIC: "/t/{slug}/{id}.json",
      POSTS: "/t/{topicId}/posts.json",
      SUBMIT_POST: "/posts",
      LIKE_POST: "/post_actions",
      UNLIKE_POST: "/post_actions/{id}",
    },
  };
    ...
    saveVisitedPosts() {
      try {
        ...
        if (this.visitedPosts.size > 200) {
          // 按时间戳排序，删除最旧的记录
          const sortedEntries = Array.from(this.visitedPosts.entries()).sort(
            (a, b) => a[1] - b[1],
          ); // 按时间戳升序排序
        ...
        mutations.forEach((mutation) => {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === Node.ELEMENT_NODE) {
              // 检查新添加的元素是否是链接或包含链接
              const links =
                node.matches && node.matches('a[href*="/t/"]')
                  ? [node]
                  : node.querySelectorAll
                    ? node.querySelectorAll('a[href*="/t/"]')
                    : [];
  ...
  const ReadingTracker = {
    ...
    isValidPost(postNumber, element) {
      ...
      // 检查是否在已加载的数据缓存中
      const isInCache = state.postDataCache.some(
        (post) => post.post_number === postNumber,
      );
    ...
    observe(commentElement) {
      ...
      if (
        postNumber &&
        this.isValidPost(parseInt(postNumber), commentElement)
      ) {
    ...
    checkInitialVisibility(element, postNumber) {
      ...
      setTimeout(() => {
        ...
        const containerRect = document
          .querySelector(".post-popup-content-wrapper")
      ...
      document.querySelectorAll("[data-post-number]").forEach((element) => {
        ...
        if (rect.top < containerRect.bottom) {
          if (!this.readingData.has(postNumber)) {
            ...
            this.addToPendingSubmissions(
              postNumber,
              this.config.minReadTime * 2,
            );
          }
        }
      });
    ...
    addToPendingSubmissions(postNumber, duration) {
      ...
      // 再次验证数据有效性
      const isValid = state.postDataCache.some(
        (post) => post.post_number === postNumber,
      );
    ...
    async submitPendingData() {
      ...
      try {
        ...
        const validData = dataToSubmit.filter((item) => {
          return state.postDataCache.some(
            (post) => post.post_number === item.postNumber,
          );
        });
    ...
    finalMarkVisibleContent() {
      ...
      document.querySelectorAll("[data-post-number]").forEach((element) => {
        ...
        if (rect.top < containerRect.top + containerRect.height * 0.8) {
          if (
            !this.readingData.has(postNumber) ||
            this.readingData.get(postNumber).duration === 0
          ) {
            const readTime = Math.max(
              this.config.minReadTime,
              Math.min(
                this.config.minReadTime * 3,
                (Date.now() - this.startTime) / 10,
              ),
            );
    ...
    async sendTimingData(readingDataArray) {
      ...
      const finalTopicTime = Math.max(
        totalTopicTime,
        scrollTime,
        totalTopicTime + scrollTime * 0.3,
      );
  ...
  GM_addStyle(`
    ...
    ${CONFIG.SELECTORS.POST_POPUP} .post-avatar { width: 40px; height: 40px; border-radius: 50%; cursor: pointer; transition: opacity 0.2s ease; }
    ${CONFIG.SELECTORS.POST_POPUP} .comment-avatar { width: 30px; height: 30px; border-radius: 50%; cursor: pointer; transition: opacity 0.2s ease; }
    ${CONFIG.SELECTORS.POST_POPUP} .post-avatar:hover,
    ${CONFIG.SELECTORS.POST_POPUP} .comment-avatar:hover { opacity: 0.8; }
    ${CONFIG.SELECTORS.POST_POPUP} .post-author,
    ${CONFIG.SELECTORS.POST_POPUP} .comment-author { font-weight: bold; color: #66aaff; cursor: pointer; transition: color 0.2s ease; }
    ${CONFIG.SELECTORS.POST_POPUP} .post-author:hover,
    ${CONFIG.SELECTORS.POST_POPUP} .comment-author:hover { color: #88ccff; text-decoration: underline; }
    ...
    /* Like Button Styles */
    .${CONFIG.CSS_CLASSES.LIKE_BTN} {
      background: none; border: none; cursor: pointer; padding: 2px 6px;
      border-radius: 4px; font-size: 14px; margin-left: 8px;
      transition: all 0.3s ease; display: inline-flex; align-items: center;
      opacity: 0.7; min-width: 24px; justify-content: center;
    }
    .${CONFIG.CSS_CLASSES.LIKE_BTN}:hover {
      background: rgba(255, 255, 255, 0.1); opacity: 1; transform: scale(1.1);
    }
    .${CONFIG.CSS_CLASSES.LIKE_BTN}:active { transform: scale(0.95); }
    .${CONFIG.CSS_CLASSES.LIKE_BTN}:disabled {
      cursor: not-allowed; opacity: 0.5; transform: none;
    }
    .${CONFIG.CSS_CLASSES.LIKE_BTN_ACTIVE} {
      opacity: 1; animation: likeAnimation 0.6s ease-out;
    }
  ...
  const Utils = {
    ...
    createPostOrCommentElement(data, isMainPost = false) {
      ...
      element.className = isMainPost
        ? CONFIG.CSS_CLASSES.POST_BOX
        : CONFIG.CSS_CLASSES.COMMENT_BOX;
      ...
      if (avatar) {
        ...
        avatar.title = `查看 ${data.username || "用户"} 的个人页面`;
        avatar.onclick = (e) => {
          e.stopPropagation();
          this.openUserProfile(data.username);
        };
      }
      ...
      author.title = `查看 ${data.username || "用户"} 的个人页面`;
      author.onclick = (e) => {
        e.stopPropagation();
        this.openUserProfile(data.username);
      };
      ...
      // 获取点赞状态
      const isLiked = Utils.extractLikeStatus(data);

      const likeBtn = document.createElement("button");
      likeBtn.className = `${CONFIG.CSS_CLASSES.LIKE_BTN} ${isLiked ? CONFIG.CSS_CLASSES.LIKE_BTN_ACTIVE : ""}`;
      likeBtn.innerHTML = isLiked ? "❤️" : "🤍";
      likeBtn.title = isLiked ? "取消点赞" : "点赞";
      ...
      likeBtn.onclick = (e) => {
        ...
        this.handleLikeClick(likeBtn, data.id, currentLiked);
      };
    ...
    async handleLikeClick(button, postId, currentLiked) {
      try {
        button.disabled = true;
        const csrfToken = Utils.getCSRFToken();
        if (!csrfToken) {
          Utils.showToast("无法获取认证信息", false);
          return;
        }
        ...
        if (currentLiked) {
          ...
          response = await fetch(
            CONFIG.API_ENDPOINTS.UNLIKE_POST.replace("{id}", postId),
            {
              method: "DELETE",
              headers: {
          ...
          formData.append("post_action_type_id", "2"); // 2 表示点赞
          formData.append("flag_topic", "false");

          response = await fetch(CONFIG.API_ENDPOINTS.LIKE_POST, {
            method: "POST",
            headers: {
              Accept: "application/json",
              "X-CSRF-Token": csrfToken,
        ...
        if (response.ok) {
          ...
          button.innerHTML = newLiked ? "❤️" : "🤍";
          button.title = newLiked ? "取消点赞" : "点赞";
          button.setAttribute("data-liked", newLiked ? "true" : "false");
          button.className = `${CONFIG.CSS_CLASSES.LIKE_BTN} ${newLiked ? CONFIG.CSS_CLASSES.LIKE_BTN_ACTIVE : ""}`;

          // 更新按钮的点击处理函数
          button.onclick = (e) => {
            e.stopPropagation();
            const currentLiked = button.getAttribute("data-liked") === "true";
            this.handleLikeClick(button, postId, currentLiked);
          };

          Utils.showToast(newLiked ? "点赞成功！" : "取消点赞成功！");
        } else {
          throw new Error(`HTTP ${response.status}: ${response.statusText}`);
        }
      } catch (error) {
        console.error("点赞操作失败:", error);
        Utils.showToast("操作失败: " + error.message, false);
      } finally {
    ...
    getCSRFToken() {
      ...
      const authenticityTokenInput = document.querySelector(
    ...
    // 提取帖子点赞状态的帮助函数
    extractLikeStatus(postData) {
      if (!postData.actions_summary) return false;
      const likeAction = postData.actions_summary.find(
        (action) => action.id === 2,
      );
      return likeAction?.acted || false;
    },

    // 打开用户个人页面
    openUserProfile(username) {
      if (!username || username === "系统") {
        Utils.showToast("无法打开系统用户页面", false);
        return;
      }

      const userUrl = `/u/${username}`;
      window.open(userUrl, "_blank");
      console.log(`打开用户页面: ${userUrl}`);
    },
  };
  ...
  const ImageLightbox = {
    ...
    show(imageSrc) {
      ...
      const lightboxContent = this.element.querySelector(
        "#image-lightbox-content",
      );
    ...
    async submitComment(topicId, content, replyToPostId = null) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const contentContainer = elements.postPopup.querySelector(
          ...
          Utils.showToast(
            `评论提交失败: ${result.errors ? result.errors.join(", ") : response.statusText}`,
            false,
          );
        }
      } catch (error) {
  ...
  const PostRenderer = {
    ...
    renderComment(comment, depth = 0) {
      ...
      if (comment.post_number && comment.id) {
        ...
        // 验证是否在缓存中，然后才注册追踪
        const isInCache = state.postDataCache.some(
          (post) => post.post_number === comment.post_number,
        );
    ...
    renderInitialPosts(posts, title, contentContainer) {
      if (posts.length > 0) {
        ...
        titleElement.style.cssText =
          "font-weight: bold; font-size: 24px; margin-bottom: 20px; color: #eee;";
  ...
  const PopupManager = {
    ...
    createButtons() {
      ...
      elements.backToTopBtn.onclick = () => {
        const contentWrapper = elements.postPopup.querySelector(
          ".post-popup-content-wrapper",
        );
        if (contentWrapper) {
      ...
      elements.openFullBtn.onclick = () => {
        if (state.currentSlug && state.currentTopicId) {
          window.open(
            `/t/${state.currentSlug}/${state.currentTopicId}`,
            "_blank",
          );
        }
      };
    ...
    async open(topicId, slug) {
      ...
      try {
        // 添加include_actions参数来获取点赞状态
        const topicUrl =
          CONFIG.API_ENDPOINTS.TOPIC.replace("{slug}", slug).replace(
            "{id}",
            topicId,
        ...
        state.nextPostIds = data.post_stream.stream.slice(
          data.post_stream.posts.length,
        );
    ...
    renderPopupContent(data) {
      ...
      PostRenderer.renderInitialPosts(
        state.postDataCache,
        data.title,
        contentContainer,
      );
      ...
      contentWrapper.addEventListener(
        ...
        Utils.throttle(() => {
          ...
          if (
            distanceFromBottom <= CONFIG.SCROLL_TRIGGER_DISTANCE &&
    ...
    async loadMorePosts(topicId, nextIds, contentContainer, editorContainer) {
      ...
      // 清理已存在的加载指示器
      let existingLoadingIndicator = contentContainer.querySelector(
        `.${CONFIG.CSS_CLASSES.LOADING_INDICATOR}`,
      );
      ...
      loadingIndicator.style.cssText =
        "text-align: center; padding: 15px; color: #66aaff; font-size: 14px;";
      contentContainer.appendChild(loadingIndicator);

      try {
        const startTime = Date.now();
        const response = await fetch(
          CONFIG.API_ENDPOINTS.POSTS.replace("{topicId}", topicId) +
        ...
        setTimeout(() => {
          ...
          // 如果还有更多内容且接近底部，继续加载
          const contentWrapper = contentContainer.closest(
            ".post-popup-content-wrapper",
          );
        ...
        loadingIndicator.style.cssText +=
          " cursor: pointer; background: #3a3a3a; border-radius: 4px; margin: 10px 0;";
